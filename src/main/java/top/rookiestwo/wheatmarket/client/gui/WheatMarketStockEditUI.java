...
public class WheatMarketStockEditUI {
    private static final ResourceLocation STOCK_EDIT_XML = ResourceLocation.parse("wheatmarket:ui/item_selection.xml");
    ...
    private final ItemStack lockedTemplate;
    ...
    public WheatMarketStockEditUI(WheatMarketMenu menu, int panelLeft, int panelTop,
                                  ItemStack lockedTemplate, Runnable finishHandler) {
        this.menu = menu;
        this.panelLeft = panelLeft;
        this.panelTop = panelTop;
        this.lockedTemplate = lockedTemplate == null ? ItemStack.EMPTY : lockedTemplate.copy();
        this.finishHandler = finishHandler == null ? () -> {
        } : finishHandler;
    }

    public ModularUI create(Player player) {
        Document xml = XmlUtils.loadXml(STOCK_EDIT_XML);
        if (xml == null) {
            throw new IllegalStateException("Failed to load UI xml: " + STOCK_EDIT_XML);
        }
    ...
    public void setFinalizing(boolean finalizing) {
        if (confirmButton != null) {
            confirmButton.setActive(!finalizing);
        }
    }
    ...
    private void bindStaticElements(UI ui) {
        ...
        Label modeLabel = require(ui, "mode-label", Label.class);
        ...
        titleLabel.setText(Component.translatable("gui.wheatmarket.stock_edit.title"));
        modeLabel.setText(Component.translatable("gui.wheatmarket.stock_edit.mode"));
        selectionSlotLabel.setText(Component.translatable("gui.wheatmarket.stock_edit.stock_slot"));
        restrictionLabel.setText(Component.translatable("gui.wheatmarket.stock_edit.hint", lockedTemplate.getHoverName()));
        ...
        confirmButton.setOnClick(event -> finishHandler.run());
        backButton.setOnClick(event -> finishHandler.run());
    }
    ...
    private void refreshSelectedItemText() {
        ...
        int selectedAmount = menu.getStockEditAmount();
        if (ItemStack.matches(seenSelectedItem, selected) && seenSelectedAmount == selectedAmount) {
            return;
        }

        seenSelectedItem = selected.copy();
        seenSelectedAmount = selectedAmount;
        selectedItemLabel.setText(selected.isEmpty()
                ? Component.translatable("gui.wheatmarket.stock_edit.final_empty")
                : Component.translatable("gui.wheatmarket.stock_edit.final_amount", selected.getHoverName(), selectedAmount));
    }
